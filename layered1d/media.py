...
class HalfSpaceMedium:
    ...
    ----------
    density:
        Mass density [kg/m^3]. Required together with ``wave_speed`` when
        ``acoustic_impedance`` is not provided.
    wave_speed:
        Longitudinal wave speed [m/s]. Required together with ``density`` when
        ``acoustic_impedance`` is not provided.
    acoustic_impedance:
        Prescribed normal-incidence impedance [kg/(m^2 s) = Pa s / m]. When
        provided alone, the medium is treated as an equivalent boundary load.
    name:
        Optional label used for logging / examples.
    """
