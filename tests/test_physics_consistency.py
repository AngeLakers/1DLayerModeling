...
class PhysicsConsistencyTests(unittest.TestCase):
    def test_material_derives_wave_speed_and_impedance(self) -> None:
        material = Material(
            density=2700.0,
            young_modulus=70e9,
            name="Aluminum",
    ...
    def test_amplitude_roundtrip_matches_boundary_displacements(self) -> None:
        material = Material(density=1200.0, young_modulus=2.2e9, name="Polymer")
    ...
    def test_reflection_is_zero_for_impedance_matched_single_layer(self) -> None:
        material = Material(density=1000.0, young_modulus=1000.0 * 1500.0 * 1500.0, name="Matched")
        layer = Layer.from_material(thickness=1.2e-3, material=material)
        water = HalfSpaceMedium(density=1000.0, wave_speed=1500.0, name="matched")
    ...
    def test_frequency_response_result_properties_and_layer_field(self) -> None:
        material = Material(density=1000.0, young_modulus=2.25e9, name="WaterLike")
    ...
    def test_lossless_energy_balance_holds_for_real_boundary_impedances(self) -> None:
        ...
        polymer = Material(density=1200.0, young_modulus=3e9, name="Polymer", poisson_ratio=0.40)
        composite = Material(density=1600.0, young_modulus=8e9, name="Composite")
        layers = [
    ...
    def test_transmission_power_is_reciprocal_under_reversal(self) -> None:
        a = Material(density=2700.0, young_modulus=70e9, name="A")
        b = Material(density=1200.0, young_modulus=3e9, name="B")
        c = Material(density=1600.0, young_modulus=8e9, name="C")
        layers = [
            Layer.from_material(thickness=0.7e-3, material=a, name="A"),
            Layer.from_material(thickness=0.4e-3, material=b, name="B"),
            Layer.from_material(thickness=1.0e-3, material=c, name="C"),
        ]
    ...
    def test_large_interface_stiffness_converges_to_rigid_reference(self) -> None:
        material_a = Material(density=2700.0, young_modulus=70e9, name="A")
        material_b = Material(density=1200.0, young_modulus=3e9, name="B")
        layers = [
            Layer.from_material(thickness=0.8e-3, material=material_a),
            Layer.from_material(thickness=0.3e-3, material=material_b),
        ]
        left = HalfSpaceMedium(density=1000.0, wave_speed=1480.0)
